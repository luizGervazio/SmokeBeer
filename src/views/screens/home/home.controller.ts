...
type DialogActionType = 'goToCart' | null;
...
export function useHomeController() {
  ...
  const showAlert = (message: string, action: DialogActionType = null) => {
  ...
  const handleAddToCart = async (wine: Wine) => {
    try {
      ...
      showAlert(`${wine.name} foi adicionado ao seu carrinho!`, 'goToCart');
    } catch (error) {
